        ...
        <div class="col-md-12">
            ...
            <section class="panel panel-info">
                <header class="panel-heading">
                    ...
                    <h2 class="panel-title"><?= __('Search'); ?> <?= __('My Receipts') ?></h2>
                </header>
                                ...
                                <?php foreach ($receipts as $receipt): ?>
                                    <tr>
                                        <td data-title="<?= __('Receipt #') ?>">
                                            <?= ($receipt->seen == 0 ? '<i class="fa fa-star-o" title="' . __('New receipt') . '"></i>' : '') ?>
                                            <?= $this->Html->link($receipt->receipt_number, ['action' => 'view', $receipt->id], ['escape' => false, 'class' => 'modalbox ', 'data-ajaxlink' => '#modalbox']) ?>
                                        </td>
                                        <td data-title="<?= __('Merchant') ?>"><?= $receipt->has('merchant') ? $this->Html->link($receipt->merchant->business_title, ['controller' => 'Merchants', 'action' => 'view', $receipt->merchant->id], ['class' => 'modalbox', 'data-ajaxlink' => '#modalbox']) : '' ?></td>
                                        ...
                                        <td data-title="<?= __('Actions') ?>" class="actions">
                                            ...
                                            if (file_exists($receiptPath . $receipt->id . '.pdf')) {
                                                echo $this->Html->link(__('Receipt'), ['action' => 'preview_receipt', $receipt->id, $receipt->security_key . '.pdf'], ['escape' => false, 'target' => '_blank', 'class' => 'modalbox_ mb-xs mt-xs mr-xs btn-xs btn-primary'/* , 'data-ajaxlink' => '#modalbox' */]);
                                            }
                                            ?>
                                        </td>
                                    </tr>
