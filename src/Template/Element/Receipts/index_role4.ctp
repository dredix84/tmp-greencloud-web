            ...
            <section class="panel panel-primary">
                ...
                <div class="panel-body">
                    <div class="_table-responsive">
                        <table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-hover mb-none table-no-more">
                            <thead>
                                <tr>
                                ...
                                <?php foreach ($receipts as $receipt): ?>
                                    <tr>
                                        <td data-title="<?= __('User') ?>">
                                            <?= $this->Html->link(h($receipt->user->username), ['controller' => 'users', 'action' => 'view', $receipt->user->id], ['escape' => false, 'class' => 'modalbox mb-xs mt-xs mr-xs', 'data-ajaxlink' => '#modalbox']) ?>
                                        </td>
                                        <td data-title="<?= __('Receipt #') ?>">
                                            <?= $this->Html->link(h($receipt->receipt_number), ['action' => 'view', $receipt->id], ['escape' => false, 'class' => 'modalbox mb-xs mt-xs mr-xs', 'data-ajaxlink' => '#modalbox']) ?>
                                        </td>
                                        ...
                                        <td data-title="<?= __('Provider') ?>"><?= $receipt->has('provider') ? $receipt->provider->name : '' ?></td>
                                        <td data-title="<?= __('Receipt Date') ?>"><?= h($receipt->receipt_date) ?></td>
                                        <td data-title="<?= __('Created') ?>"><?= h($receipt->created) ?></td>
                                        <td data-title="<?= __('Total') ?>"><?= __('$') . h($receipt->total) ?></td>
                                        <td data-title="<?= __('Total') ?>"><?= __('$') . h($receipt->balance) ?></td>
                                        <td data-title="<?= __('Loyalty') ?>"><?= __('$') . h($receipt->loyalty_cost) ?></td>
                                        <td data-title="<?= __('Credits') ?>"><?= $receipt->credits_used ?></td>
